...
function url(src, change_quality) {
    ...
    if (/\.mpd/.test(src) && typeof dashjs !== 'undefined') {
        try {
            if (Platform.is('orsay') && Storage.field('player') == 'orsay') { load(src) }
            else {
        ...
        if (typeof Hls !== 'undefined') {
            ...
            //для Apple TV принудительно используем HLS.js если доступно для избежания десинхронизации аудио/видео
            else if (Platform.is('apple_tv') && Hls.isSupported()) use_program = true
            ...
            console.log('Player', 'use program hls:', use_program, 'hlsjs:', Hls.isSupported(), 'platform:', Platform.any() ? Platform.any() : 'unknown')
