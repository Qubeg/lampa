    ...
    const promise = (async () => {
        try {
            ...
            const episodes = await new Promise((resolve, reject) => {
                ...
                Timetable.get({id: parseInt(tvId)}, (result) => {
                    abortController.signal.removeEventListener('abort', onAbort)
                    // Фильтруем эпизоды нужного сезона
                    const filtered = (result || []).filter(ep => ep.season_number === parseInt(season))
                    resolve(filtered.length ? filtered : null)
                })
            })
    ...
    return new Promise(resolve => {
        ...
        getShowMetaFromCache(data, { abortKey: options.abortKey }).then(tvShowData => {
            ...
            doScan.then(found => {
                ...
                const currentSeasonInfo = findSeasonByNumber(curSeason) || { episode_count: 0 }
                
                // Базовая проверка: последний ли это эпизод в самом последнем сезоне
                let lastOfAll = curSeason === lastSeasonNum && curEpisode >= (lastSeasonInfo.episode_count || 0)

                // Если текущий сезон просмотрен, но есть следующий — проверяем, вышел ли он
                if(!lastOfAll && curEpisode >= (currentSeasonInfo.episode_count || 0)){
                    const nextSeason = getNextSeason(curSeason)
                    if(nextSeason && (!nextSeason.air_date || Utils.countDays(Date.now(), nextSeason.air_date) > 0)){
                        lastOfAll = true
                    }
                }
...
function createItem(text, classes = [], showTimeline = false, timeline = null){
    const children = [Template.elem('span', { text })]
    
    if(showTimeline && timeline){
        children.push(Timeline.render(timeline)[0])
    }

    return Template.elem('div', {
        class: 'card-watched__item' + (classes.length ? ' ' + classes.join(' ') : ''),
        children
    })
}
...
function render(plan, opts = {}){
    ...
    const body = wrap.find('.card-watched__body')

    if(plan.type === 'movie'){
        body.append(createItem(plan.current.name, [], options.withTimeline, plan.view))
    } else {
            ...
            if(missedList?.length){
                ...
                items.push(createItem(Lang.translate('missed_episodes') + ': ' + missedLabel, ['card-watched__missed']))
            }
            ...
            if(lastOfAll){
                items.push(createItem(Lang.translate('last_episode_now'), ['card-watched__final']))
            }
            ...
            Promise.allSettled(seasonsArray.map(season => fetchSeasonFromCache(tvId, season, { abortKey: options.abortKey })))
                .then((results) => {
                    ...
                    nodes.forEach(node => {
                        ...
                        const span = node.node.find('span')
                        if(!span) return
    ...
    if(options.mount){
        if(options.position === 'append') options.mount.append(wrap)
        else options.mount.prepend(wrap)
    }
...
/**
 * Проверить, есть ли прогресс просмотра для карточки (синхронно)
 * @param {Object} data 
 * @returns {boolean}
 */
function check(data){
    ...
    if(data.original_name){
        // Для сериалов проверяем кэшированную позицию
        const positionCache = Storage.cache('watched_position_cache', CACHE_LIMIT_WATCHED_POSITION, {})
        if(positionCache[data.id]) return true
        
        // Или ищем в таймлайне (хотя бы первые несколько эпизодов/сезонов)
        // Это не идеально, но для иконки достаточно
        for(let s = 1; s <= 2; s++){
            for(let e = 1; e <= 20; e++){
                if(viewed[hashEpisode(data.original_title, s, e)]) return true
            }
        }
    } else {
        const time = viewed[Utils.hash(data.original_title)]
        if(typeof time === 'object') return (time.percent || 0) > 0
